---
title: Pregled enotne prijave (SSO) | digna Dokumentacija
description: Kako deluje enotna prijava (SSO) v digna z uporabo OpenID Connect (OIDC). Vsebuje konfiguracijo nadzorne plošče in zaledja, testiranje, odpravljanje težav in povezave do navodil za posamezne ponudnike, kot so Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne in AD FS.
...
Za vsakega ponudnika mora obstajati namenski razdelek `[oidc_clients.<key>]`. Ključ se mora ujemati s `key`, določenim v `dashboard_config.toml`.
