---
title: Vue d'ensemble des connexions aux bases de données – Configuration ODBC sans DSN | digna Documentation
description: Comment fonctionnent les connexions aux bases de données dans digna. Chaque technologie source est atteinte via ODBC avec une chaîne de connexion sans DSN construite à partir de propriétés ODBC. Couvre l'installation du pilote sur l'hôte digna, l'écran Add DB Connection, le chiffrement des propriétés, le test des connexions, le dépannage et les liens vers les guides par technologie.
...
lang: fr
robots: index, follow
og_title: Connexions aux bases de données digna – Configuration ODBC sans DSN
og_description: Configurez une connexion source digna via ODBC sans DSN. Installation du pilote, propriétés ODBC, chiffrement, tests et dépannage.
