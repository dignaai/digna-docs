---
title: Présentation de l’authentification unique (SSO) | Documentation digna
description: Fonctionnement de l’authentification unique dans digna via OpenID Connect (OIDC). Couvre la configuration du tableau de bord et du backend, les tests, le dépannage et liens vers des guides par fournisseur pour Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne et AD FS.
...
Chaque fournisseur doit avoir une section dédiée `[oidc_clients.<key>]`. La key doit correspondre à la `key` définie dans `dashboard_config.toml`.
