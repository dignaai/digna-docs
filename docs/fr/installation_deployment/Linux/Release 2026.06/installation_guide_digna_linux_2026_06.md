...
!!! note "Le serveur occupe le terminal"
    ...
    `serve` s'exécute au premier plan et continue de tourner jusqu'à ce que vous l'arrêtiez avec ++ctrl+c++. Laissez-le tourner pendant que vous terminez la configuration ; pour le démarrer automatiquement au démarrage, voir [Exécuter digna en tant que service systemd](#running-digna-as-a-systemd-service).
