---
title: نظرة عامة على تسجيل الدخول الأحادي (SSO) | digna Documentation
description: كيف يعمل تسجيل الدخول الأحادي في digna باستخدام OpenID Connect (OIDC). يغطي إعداد لوحة القيادة والـ backend والاختبار واستكشاف الأخطاء، وروابط لأدلة إعداد لكل مزود مثل Microsoft Entra ID وGoogle Workspace وOkta وAuth0 وKeycloak وOneLogin وPingOne وAD FS.
...
يجب أن يكون لكل مزود قسم مخصص `[oidc_clients.<key>]`. يجب أن يتطابق المفتاح مع `key` المعرفة في `dashboard_config.toml`.
