---
title: Przegląd Single Sign-On (SSO) | dokumentacja digna
description: Jak działa Single Sign-On w digna przy użyciu OpenID Connect (OIDC). Zawiera konfigurację dashboardu i backendu, testowanie, rozwiązywanie problemów oraz odnośniki do przewodników dla Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne i AD FS.
...
lang: pl
robots: index, follow
og_title: Integracja Single Sign-On (SSO) w digna — przewodnik
...
Każdy dostawca musi mieć dedykowaną sekcję `[oidc_clients.<key>]`. Klucz musi pasować do `key` zdefiniowanego w `dashboard_config.toml`.
