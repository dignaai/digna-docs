---
title: Οδηγός Εγκατάστασης Windows – digna Release 2026.06 | Τεκμηρίωση digna
description: Βήμα‑προς‑βήμα οδηγός για την εγκατάσταση του digna Release 2026.06 σε Windows — απαιτήσεις συστήματος, ρύθμιση PostgreSQL, ρύθμιση web server, διαμόρφωση backend και dashboard, εκτέλεση digna ως υπηρεσία Windows και αναβάθμιση σε νέα έκδοση.
keywords: εγκατάσταση digna σε windows, οδηγός ανάπτυξης digna, ρύθμιση digna backend, εγκατάσταση digna dashboard, ρύθμιση postgresql, digna υπηρεσία windows, οδηγός αναβάθμισης digna
...
- **digna**: ο πυρήνας της εφαρμογής, υπεύθυνος για την επεξεργασία δεδομένων και την εκτέλεση ελέγχων ποιότητας. Ενοποιεί το backend και τη διεπαφή γραμμής εντολών σε ένα μόνο εκτελέσιμο αρχείο, αντικαθιστώντας τα ξεχωριστά προγράμματα `dignabackend` και `dignacli` των προηγούμενων εκδόσεων.
...
| `DIGNA_SCHEDULER_MAX_DELAY` | `100` | Μέγιστη καθυστέρηση, σε δευτερόλεπτα, που μπορεί να προσθέσει ο χρονοπρογραμματιστής πριν ξεκινήσει μια εργασία που έχει καταστεί ληξιπρόθεσμη |
| `DIGNA_CLEANUP_TIME` | `"12:00"` | Ώρα της ημέρας (24ωρη μορφή `HH:MM`) κατά την οποία ξεκινά ο καθημερινός καθαρισμός |

#### Ενότητα [encryption]

Αυτή η ενότητα περιέχει το κλειδί που χρησιμοποιείται για την κρυπτογράφηση ευαίσθητων τιμών που αποθηκεύονται στο repository. Είναι **υποχρεωτική** — το `config check` αναφέρει την ενότητα `[encryption]` ως FAILED αν λείπει το κλειδί.
...
| Παράμετρος | Τιμή | Σημειώσεις |
|---|---|---|
| `DIGNA_ENCRYPTION_KEY` | Κλειδί κωδικοποιημένο σε Base64 | Κρυπτογραφεί ευαίσθητες τιμές που αποθηκεύονται στο repository digna |

!!! warning "Προστατέψτε το config.toml"

    Αυτό το κλειδί είναι σταθερή τιμή, ίδια σε όλες τις εγκαταστάσεις digna, και είναι αυτό που αποκρυπτογραφεί τις ευαίσθητες τιμές του repository σας. Περιορίστε το `config.toml` στον λογαριασμό που εκτελεί το digna, κρατήστε το αρχείο εκτός συστήματος ελέγχου εκδόσεων και κοινόχρηστων δίσκων και εξαιρέστε το από κάθε αντίγραφο ασφαλείας που φυλάσσεται λιγότερο ασφαλώς από το ίδιο το repository.
...
### Βήμα 2: Επικυρώστε τη διαμόρφωση

Πριν αρχικοποιήσετε το repository, ελέγξτε ότι το `config.toml` είναι πλήρες και σωστά δομημένο. Στον κατάλογο εγκατάστασης digna εκτελέστε:

```bash
digna config check
```

Κάθε ενότητα επικυρώνεται ξεχωριστά, ώστε ένα μεμονωμένο σφάλμα να μην κρύβει την κατάσταση των υπολοίπων:
...
Διορθώστε ό,τι αναφέρεται ως FAILED και εκτελέστε ξανά την εντολή πριν συνεχίσετε. Ο πλήρης κατάλογος επιλογών βρίσκεται στην [αναφορά CLI](../../../cli/Command_Line_Interface_202606.md).
...
### Βήμα 5: Δημιουργία Χρήστη Διαχειριστή
...
### Βήμα 6: Εκκίνηση του digna Server
...
!!! note "Ο διακομιστής δεσμεύει το τερματικό"

    Το `serve` εκτελείται στο προσκήνιο και συνεχίζει μέχρι να το σταματήσετε με ++ctrl+c++. Αφήστε το να εκτελείται όσο ολοκληρώνετε την εγκατάσταση· για αυτόματη εκκίνηση κατά την εκκίνηση του συστήματος, δείτε [Εκτέλεση του digna ως υπηρεσίας Windows](#running-digna-as-a-windows-service).
...
**Ελέγξτε πρώτα όλες τις συνδέσεις βάσεων δεδομένων**

Από την έκδοση 2026.06, το digna προσεγγίζει κάθε τεχνολογία πηγής μέσω **ODBC**. Οι προηγούμενες εκδόσεις πρόσφεραν επιλογή ανάμεσα σε πρόγραμμα οδήγησης ανά τεχνολογία και ODBC, μέσω του διακόπτη **Use ODBC**. Η ομάδα digna αποφάσισε να στηριχθεί αποκλειστικά στο ODBC, επειδή μία μόνο τυποποιημένη διεπαφή προσφέρει περισσότερα από ένα σύνολο προγραμμάτων οδήγησης κατά παραγγελία:

- **Έλεγχος ταυτότητας** — ο έλεγχος ταυτότητας αποτελεί μέρος του ODBC, οπότε μια σύνδεση μπορεί να χρησιμοποιήσει οτιδήποτε υποστηρίζει το πρόγραμμα οδήγησής της: κωδικούς πρόσβασης, tokens και PAT, Kerberos και Active Directory, MFA και ενιαία σύνδεση μέσω προγράμματος περιήγησης, ταυτότητες cloud, πιστοποιητικά πελάτη και TLS. Οι νέες μέθοδοι έρχονται με ενημέρωση του προγράμματος οδήγησης, αντί να περιμένουν μια έκδοση του digna.
- **Προγράμματα οδήγησης που συντηρούνται από τους κατασκευαστές των βάσεων δεδομένων** — το πρόγραμμα οδήγησης του ίδιου του κατασκευαστή παρακολουθεί νέες εκδόσεις διακομιστή και διορθώσεις ασφαλείας, και μπορείτε να το ενημερώνετε με το δικό σας πρόγραμμα, ανεξάρτητα από το digna.
- **Ένας τρόπος για να διαμορφώνετε τα πάντα** — κάθε τεχνολογία είναι μια λίστα ιδιοτήτων κλειδιού/τιμής, με την ίδια διεπαφή, την ίδια κρυπτογράφηση ευαίσθητων τιμών και την ίδια αντιμετώπιση προβλημάτων, αντί για διαφορετικό σύνολο πεδίων ανά πηγή.
- **Ρύθμιση και εμβέλεια** — επιλογές σε επίπεδο προγράμματος οδήγησης, όπως χρονικά όρια, ρυθμίσεις TLS, διακομιστές μεσολάβησης και μεγέθη ανάκτησης, είναι διαθέσιμες για κάθε πηγή, και μπορεί να συνδεθεί οποιαδήποτε τεχνολογία διαθέτει συμβατό πρόγραμμα οδήγησης ODBC, ακόμη και εκείνες για τις οποίες το digna δεν δημοσιεύει ειδικό οδηγό.

Στην πράξη αυτό σημαίνει ότι ο διακόπτης **Use ODBC** και τα ξεχωριστά πεδία κεντρικού υπολογιστή, θύρας, βάσης δεδομένων, χρήστη και κωδικού πρόσβασης δεν υπάρχουν πλέον. **Κάθε σύνδεση που δεν χρησιμοποιεί ήδη ODBC πρέπει να μεταφερθεί σε ODBC** — δεν υπάρχει αυτόματη μετατροπή, οπότε προγραμματίστε το πριν από την αναβάθμιση:

1. Εξετάστε κάθε σύνδεση βάσης δεδομένων που ορίζεται στην εγκατάστασή σας και σημειώστε εκείνες που δεν χρησιμοποιούν ακόμη ODBC — καθεμιά από αυτές θα πρέπει να διαμορφωθεί εκ νέου.
2. Εγκαταστήστε το αντίστοιχο πρόγραμμα οδήγησης ODBC στον κεντρικό υπολογιστή digna — οι συνδέσεις ανοίγουν από τον διακομιστή που εκτελεί το backend του digna, όχι από το πρόγραμμα περιήγησης. Δείτε [Εγκατάσταση του προγράμματος οδήγησης ODBC στον κεντρικό υπολογιστή digna](../../../databases/overview.md#install-the-driver).
3. Έχετε έτοιμες τις ιδιότητες ODBC για κάθε επηρεαζόμενη σύνδεση. Οι [οδηγοί ανά τεχνολογία](../../../databases/overview.md#technology-guides) παραθέτουν για κάθε πηγή ένα δοκιμασμένο σύνολο ιδιοτήτων.

Μετά την αναβάθμιση, μεταφέρετε κάθε επηρεαζόμενη σύνδεση σε ODBC και δοκιμάστε την από τον πίνακα ελέγχου — δείτε [Δημιουργία σύνδεσης βάσης δεδομένων](../../../databases/overview.md#create-a-database-connection) και [Δοκιμή σύνδεσης](../../../databases/overview.md#testing-a-connection).

!!! warning "Συνδέσεις Databricks Legacy"

    Ο σύνδεσμος Databricks Legacy καταργήθηκε σε αυτή την έκδοση. Μεταφέρετε αυτές τις συνδέσεις στον σύνδεσμο [Databricks](../../../databases/databricks_connector_guide.md).
...
#### Βήμα 2: Δημιουργήστε αντίγραφο της τρέχουσας εγκατάστασης

Στον κατάλογο εγκατάστασης digna, μετονομάστε τους φακέλους της τρέχουσας εγκατάστασης, ώστε η νέα έκδοση να μπορεί να αναπτυχθεί δίπλα τους:
...
!!! info "τα dignabackend και dignacli δεν χρησιμοποιούνται πλέον"

    Από την έκδοση 2026.06, τα `dignabackend` και `dignacli` αντικαθίστανται από το ενιαίο εκτελέσιμο `digna`, που ενοποιεί το backend και το CLI. Κρατήστε τα `dignabackend_old` και `dignacli_old` μόνο μέχρι να επαληθεύσετε την αναβάθμιση — μετά μπορείτε να διαγράψετε και τους δύο φακέλους. Κρατήστε τον `dashboard_old` μέχρι να επαναφέρετε από αυτόν τα αρχεία διαμόρφωσής σας (δείτε το βήμα 4).
...
Κάθε ενότητα πρέπει να αναφέρει OK. Διορθώστε ό,τι αναφέρεται ως FAILED και εκτελέστε ξανά την εντολή πριν συνεχίσετε.

#### Βήμα 6: Αναβάθμιση του Schema του Repository
...
#### Βήμα 7: Επανεκκίνηση των Υπηρεσιών
...
#### Βήμα 8: Επαλήθευση της Αναβάθμισης

1. Πρόσβαση στο digna dashboard
2. Επαληθεύστε ότι η διεπαφή φορτώνει σωστά
3. Ελέγξτε τα logs του server για τυχόν σφάλματα
4. Μεταφέρετε σε ODBC κάθε σύνδεση που δεν το χρησιμοποιούσε ακόμη και έπειτα δοκιμάστε όλες τις συνδέσεις — δείτε [Δοκιμή σύνδεσης](../../../databases/overview.md#testing-a-connection)