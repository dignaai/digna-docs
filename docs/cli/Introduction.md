...
**digna*cli* folder, and configure your connection to the ***digna*** repository. Ensure you have 
...
2. **Deploying the `*digna*cli` Folder:**
   - Copy the `*digna*cli` folder to your preferred installation location (e.g., `C:\Program Files\***digna***`).

3. **Configuring `config.toml`:**
   - Check for `config.toml` inside `*digna*cli`.
