---
title: Vienreizējā autentifikācija (SSO) — pārskats | digna dokumentācija
description: Kā vienreizējā autentifikācija darbojas digna, izmantojot OpenID Connect (OIDC). Ietver informāciju par informācijas paneļa un backend konfigurāciju, testēšanu, problēmu novēršanu un saites uz katra sniedzēja uzstādīšanas rokasgrāmatām Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne un AD FS.
image: /assets/logo_square.png
keywords:
  - digna sso
  - vienreizējā autentifikācija
  - OIDC integrācija
  - OpenID Connect
  - Microsoft Entra ID
  - Azure AD SSO
...
Katram sniedzējam jābūt atsevišķai `[oidc_clients.<key>]` sadaļai. Key jāatbilst `key`, kas definēts `dashboard_config.toml`.
