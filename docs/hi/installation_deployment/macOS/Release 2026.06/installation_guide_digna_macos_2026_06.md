...
- **digna**: एप्लिकेशन का केंद्र, जो डेटा संसाधित करने और गुणवत्ता जाँच चलाने के लिए ज़िम्मेदार है। यह बैकएंड और कमांड लाइन इंटरफ़ेस को एक ही निष्पादन योग्य फ़ाइल में जोड़ता है और पिछली रिलीज़ के अलग-अलग प्रोग्राम `dignabackend` तथा `dignacli` की जगह लेता है।
...
!!! warning "महत्वपूर्ण — macOS यहाँ Windows से अलग है"
    ...
    इसका मतलब है कि एक फ्रेश Homebrew इंस्टॉलेशन पर `postgres` रोल मौजूद नहीं होता। जब आपको सुपरयूज़र की आवश्यकता हो तब अपने अकाउंट नाम का उपयोग करें, और [प्रारम्भिक इंस्टॉलेशन](#initial-installation) में बताए अनुसार एक स्पष्ट digna यूज़र बनाएं।
...
| `DIGNA_SCHEDULER_MAX_DELAY` | `100` | अधिकतम विलंब, सेकंड में, जिसे शेड्यूलर किसी नियत कार्य को शुरू करने से पहले जोड़ सकता है |
| `DIGNA_CLEANUP_TIME` | `"12:00"` | दिन का वह समय (24-घंटे का `HH:MM` प्रारूप) जब दैनिक सफ़ाई शुरू होती है |
...
इस सेक्शन में वह कुंजी होती है जिससे रिपॉज़िटरी में संग्रहीत संवेदनशील मान एन्क्रिप्ट किए जाते हैं। यह **आवश्यक** है — कुंजी न होने पर `config check` सेक्शन `[encryption]` को FAILED बताता है।
...
| पैरामीटर | मान | नोट्स |
|---|---|---|
| `DIGNA_ENCRYPTION_KEY` | Base64 में एन्कोड की गई कुंजी | digna रिपॉज़िटरी में संग्रहीत संवेदनशील मानों को एन्क्रिप्ट करती है |

!!! warning "config.toml की सुरक्षा करें"

    यह कुंजी एक निश्चित मान है, जो सभी digna संस्थापनों में एक समान है, और यही आपकी रिपॉज़िटरी के संवेदनशील मानों को डिक्रिप्ट करती है। `config.toml` तक पहुँच उसी खाते तक सीमित रखें जिससे digna चलता है, फ़ाइल को संस्करण नियंत्रण और साझा ड्राइव से बाहर रखें, और उसे हर ऐसे बैकअप से हटा दें जो रिपॉज़िटरी की तुलना में कम सुरक्षित रखा जाता है।
...
### चरण 2: कॉन्फ़िगरेशन जाँचें

रिपॉज़िटरी आरंभ करने से पहले जाँचें कि `config.toml` पूर्ण और सही ढंग से बना है। अपनी digna संस्थापन निर्देशिका में चलाएँ:

```bash
digna config check
```

प्रत्येक सेक्शन की जाँच अलग से होती है, इसलिए एक अकेली गलती बाकी की स्थिति नहीं छिपाती:
...
जो कुछ भी FAILED बताया जाए उसे ठीक करें और आगे बढ़ने से पहले कमांड फिर चलाएँ। विकल्पों की पूरी सूची [CLI संदर्भ](../../../cli/Command_Line_Interface_202606.md) में है।

### चरण 3: Repository इनिशियलाइज़ करें
...
### चरण 4: Repository Schema इंस्टॉल करें
...
!!! note "सर्वर टर्मिनल को व्यस्त रखता है"

    `serve` अग्रभूमि में चलता है और तब तक चलता रहता है जब तक आप इसे ++ctrl+c++ से रोक नहीं देते। सेटअप पूरा करते समय इसे चलता रहने दें; इसके बजाय बूट पर स्वतः प्रारंभ करने के लिए देखें [digna को पृष्ठभूमि सेवा के रूप में चलाना](#running-digna-as-a-background-service).
...
**पहले सभी डेटाबेस कनेक्शन जाँचें**

रिलीज़ 2026.06 से digna हर स्रोत तकनीक तक **ODBC** के माध्यम से पहुँचता है। पिछली रिलीज़ में प्रति तकनीक अलग ड्राइवर और ODBC के बीच चुनाव था, जिसे **Use ODBC** स्विच से तय किया जाता था। digna टीम ने केवल ODBC पर आधारित रहने का निर्णय लिया, क्योंकि एक ही मानक इंटरफ़ेस विशेष रूप से बनाए गए ड्राइवरों के समूह से अधिक देता है:

- **प्रमाणीकरण** — प्रमाणीकरण ODBC का हिस्सा है, इसलिए कोई कनेक्शन वह सब उपयोग कर सकता है जो उसका ड्राइवर समर्थित करता है: पासवर्ड, टोकन और PAT, Kerberos और Active Directory, MFA और ब्राउज़र-आधारित सिंगल साइन-ऑन, क्लाउड पहचान, क्लाइंट प्रमाणपत्र और TLS। नई विधियाँ digna की रिलीज़ की प्रतीक्षा किए बिना, ड्राइवर अपडेट के साथ आती हैं।
- **डेटाबेस निर्माताओं द्वारा अनुरक्षित ड्राइवर** — निर्माता का अपना ड्राइवर नए सर्वर संस्करणों और सुरक्षा सुधारों का अनुसरण करता है, और आप उसे digna से स्वतंत्र रूप से, अपनी सुविधा के अनुसार अपडेट कर सकते हैं।
- **सब कुछ कॉन्फ़िगर करने का एक ही तरीका** — हर तकनीक कुंजी/मान गुणों की एक सूची है, उसी इंटरफ़ेस, संवेदनशील मानों के उसी एन्क्रिप्शन और उसी समस्या-निवारण के साथ, न कि प्रति स्रोत अलग-अलग फ़ील्ड।
- **समायोजन और पहुँच** — ड्राइवर-स्तर के विकल्प जैसे टाइमआउट, TLS सेटिंग्स, प्रॉक्सी और फ़ेच आकार हर स्रोत के लिए उपलब्ध हैं, और अनुरूप ODBC ड्राइवर वाली कोई भी तकनीक जोड़ी जा सकती है, वे भी जिनके लिए digna अलग मार्गदर्शिका प्रकाशित नहीं करता।

व्यवहार में इसका अर्थ है कि **Use ODBC** स्विच तथा होस्ट, पोर्ट, डेटाबेस, उपयोगकर्ता और पासवर्ड के अलग फ़ील्ड अब मौजूद नहीं हैं। **हर वह कनेक्शन जो पहले से ODBC का उपयोग नहीं करता, उसे ODBC में बदलना होगा** — कोई स्वचालित रूपांतरण नहीं है, इसलिए अपग्रेड से पहले इसकी योजना बनाएँ:

1. अपने संस्थापन में परिभाषित प्रत्येक डेटाबेस कनेक्शन की समीक्षा करें और उन्हें नोट करें जो अभी ODBC का उपयोग नहीं करते — इनमें से हर एक को फिर से कॉन्फ़िगर करना होगा।
2. संबंधित ODBC ड्राइवर को digna होस्ट पर संस्थापित करें — कनेक्शन ब्राउज़र से नहीं, बल्कि उस सर्वर से खोले जाते हैं जिस पर digna बैकएंड चलता है। देखें [digna होस्ट पर ODBC ड्राइवर संस्थापित करना](../../../databases/overview.md#install-the-driver)।
3. प्रभावित प्रत्येक कनेक्शन के लिए ODBC गुण तैयार रखें। [तकनीक-वार मार्गदर्शिकाएँ](../../../databases/overview.md#technology-guides) प्रत्येक स्रोत के लिए एक परखा हुआ गुण-समूह देती हैं।

अपग्रेड के बाद प्रभावित प्रत्येक कनेक्शन को ODBC में बदलें और डैशबोर्ड से उसका परीक्षण करें — देखें [डेटाबेस कनेक्शन बनाना](../../../databases/overview.md#create-a-database-connection) और [कनेक्शन का परीक्षण](../../../databases/overview.md#testing-a-connection)।

!!! warning "Databricks Legacy कनेक्शन"

    Databricks Legacy कनेक्टर इस रिलीज़ में हटा दिया गया है। उन कनेक्शनों को [Databricks](../../../databases/databricks_connector_guide.md) कनेक्टर पर स्थानांतरित करें।
...
#### चरण 2: वर्तमान संस्थापन का बैकअप लें

अपनी digna संस्थापन निर्देशिका में वर्तमान संस्थापन के फ़ोल्डरों के नाम बदलें, ताकि नई रिलीज़ उनके साथ तैनात की जा सके:
...
!!! info "dignabackend और dignacli अब उपयोग में नहीं हैं"

    रिलीज़ 2026.06 से `dignabackend` और `dignacli` की जगह एकल निष्पादन योग्य फ़ाइल `digna` ले लेती है, जो बैकएंड और CLI को जोड़ती है। `dignabackend_old` और `dignacli_old` को केवल तब तक रखें जब तक आप अपग्रेड की पुष्टि न कर लें — उसके बाद दोनों फ़ोल्डर हटाए जा सकते हैं। `dashboard_old` को तब तक रखें जब तक आप उससे अपनी कॉन्फ़िगरेशन फ़ाइलें पुनर्स्थापित न कर लें (चरण 4 देखें)।
...
प्रत्येक सेक्शन को OK बताना चाहिए। जो कुछ भी FAILED बताया जाए उसे ठीक करें और आगे बढ़ने से पहले कमांड फिर चलाएँ।

#### चरण 6: Repository Schema को अपग्रेड करें
...
#### चरण 7: सर्विसेज़ पुनः आरंभ करें
...
4. हर उस कनेक्शन को ODBC में बदलें जो अभी तक इसका उपयोग नहीं करता था, फिर सभी कनेक्शनों का परीक्षण करें — देखें [कनेक्शन का परीक्षण](../../../databases/overview.md#testing-a-connection)