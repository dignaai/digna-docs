---
title: シングルサインオン (SSO) 概要 | digna ドキュメント
description: OpenID Connect (OIDC) を使った digna のシングルサインオンの仕組み。ダッシュボードとバックエンドの設定、テスト、トラブルシューティング、Microsoft Entra ID、Google Workspace、Okta、Auth0、Keycloak、OneLogin、PingOne、AD FS 各プロバイダー別セットアップガイドへのリンクを含みます。
