---
title: digna リリース 2026.06 の Linux インストールガイド | digna ドキュメント
description: Linux に digna リリース 2026.06 をインストールするためのステップバイステップガイド — システム要件、PostgreSQL の設定、nginx または Apache の構成、バックエンドとダッシュボードの設定、systemd サービスとしての digna の実行、リリースへのアップグレード方法
keywords: digna linux インストール, digna 展開ガイド, digna バックエンド 設定, digna ダッシュボード インストール, postgresql linux, nginx linux, digna systemd サービス, digna アップグレード ガイド
...
!!! note "サーバーはターミナルを占有します"
    ...
    `serve` はフォアグラウンドで実行され、++ctrl+c++ で停止するまで動作し続けます。セットアップを終えるまで実行したままにしてください。起動時に自動的に開始する方法は次を参照してください: [digna を systemd サービスとして実行する](#running-digna-as-a-systemd-service).
