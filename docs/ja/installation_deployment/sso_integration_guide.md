---
title: シングルサインオン（SSO）統合ガイド | digna ドキュメント
description: OpenID Connect (OIDC) を使用して digna のシングルサインオン（SSO）を構成する手順ガイド。ダッシュボードとバックエンドの設定、テスト、トラブルシューティング、Microsoft Entra ID、Google Workspace、Okta などのサポートプロバイダを網羅。
...
lang: ja
robots: index, follow
og_title: digna シングルサインオン（SSO）統合ガイド
og_description: OpenID Connect を使用して digna のシングルサインオンを構成します。Microsoft Entra ID、Google Workspace、Okta、およびその他の OIDC 準拠のアイデンティティプロバイダ向けのステップバイステップ設定。
