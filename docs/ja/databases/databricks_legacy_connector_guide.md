---
title: Databricks コネクター（レガシー、Unity Catalogなし） | digna ドキュメント
description: ネイティブの Python コネクタまたは Simba Spark ODBC ドライバを使用して Unity Catalog なしの Databricks に接続するよう digna を設定します。トークンベース認証と柔軟な接続性をサポートします。
...
> ⚠️ 他の認証方法を利用する場合は、ODBC ドライバを使用してください。
