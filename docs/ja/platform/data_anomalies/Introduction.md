---
title: Data Anomalies – データの異常を自動検出 | dignaドキュメント
description: digna Data Anomalies がルールを手動で書くことなく、ボリュームの急落、欠損値、分布の変化、予期しないパターンを自動検出する仕組みを学びます。AI駆動の異常検出により、データパイプラインのデータ品質と可観測性を向上させます。
...
lang: ja
robots: index, follow
og_title: Data Anomalies – 自動検出 | dignaドキュメント
og_description: digna Data Anomalies は、手動ルール不要でデータのボリューム、値、分布の異常を自動検出し、プラットフォーム全体でデータ品質と可観測性を改善します。
