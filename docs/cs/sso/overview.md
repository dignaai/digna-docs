---
title: Přehled Single Sign-On (SSO) | digna Dokumentace
description: Jak funguje Single Sign-On v digna pomocí OpenID Connect (OIDC). Zahrnuje konfiguraci dashboardu a backendu, testování, odstraňování problémů a odkazy na průvodce pro Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne a AD FS.
...
Každý poskytovatel musí mít dedikovanou sekci `[oidc_clients.<key>]`. Klíč musí odpovídat `key` definovanému v `dashboard_config.toml`.
