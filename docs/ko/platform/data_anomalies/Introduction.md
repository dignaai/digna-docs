---
title: Data Anomalies – 데이터 불규칙성 자동 감지 | digna 문서
description: digna Data Anomalies가 수동 규칙 코딩 없이 볼륨 감소, 결측값, 분포 변화 및 예기치 않은 패턴을 자동으로 감지하는 방법을 알아보세요. AI 기반 이상 탐지로 데이터 파이프라인의 데이터 품질과 관측성을 향상시키세요.
...
lang: ko
robots: index, follow
og_title: Data Anomalies – 자동 감지 | digna 문서
og_description: digna Data Anomalies는 수동 규칙 없이 데이터 볼륨, 값 및 분포의 불규칙성을 자동으로 감지하여 플랫폼 전반의 데이터 품질과 관측성을 개선합니다.
