---
title: PostgreSQL 커넥터 – 데이터베이스 통합 | digna 문서
description: psycopg Python 드라이버 또는 PostgreSQL ODBC 드라이버를 사용하여 digna를 PostgreSQL에 연결하도록 구성하는 방법입니다. DSN 또는 DSN-less 설정에서 암호 기반 인증을 지원합니다.
...
> ⚠️ 다른 인증 방법을 사용해야 하는 경우 ODBC 드라이버를 사용하세요.
