---
title: macOS 설치 안내 – digna Release 2026.06 | digna 문서
description: macOS에 digna Release 2026.06을 설치하는 단계별 안내 — 시스템 요구사항, Homebrew 및 PostgreSQL 설정, nginx 또는 Apache 구성, 백엔드 및 대시보드 설정, digna를 백그라운드 서비스로 실행 및 새 릴리스로 업그레이드 방법.
keywords: digna macos 설치, digna mac 배포 안내, digna 백엔드 설정, digna 대시보드 설치, postgresql homebrew, nginx macos, digna launchd 서비스, digna 업그레이드 안내
...
!!! note "서버가 터미널을 점유합니다"
    ...
    `serve`는 포그라운드에서 실행되며 ++ctrl+c++로 중지할 때까지 계속 동작합니다. 설정을 마칠 때까지 실행 상태로 두십시오. 대신 부팅 시 자동으로 시작하려면 다음을 참조하십시오: [digna를 백그라운드 서비스로 실행](#running-digna-as-a-background-service).
