---
title: 데이터베이스 연결 | digna 문서
description: digna에서 기존 프로젝트에 데이터베이스를 연결하는 단계별 가이드입니다. 연결 선택, 설정 구성 및 보안 액세스 활성화 방법을 알아보세요.
image: /assets/logo_square.png
---

# 프로젝트에 Datasource (Table) 추가

이 가이드는 프로젝트에 Datasource를 추가하는 최소 단계들을 보여줍니다.

## Interactive Demo

<!--ARCADE EMBED START--><div style="position: relative; padding-bottom: calc(45.27777777777778% + 41px); height: 0; width: 100%;"><iframe src="https://demo.arcade.software/jvxy4tXv5xQlRAa1MsLI?embed&embed_mobile=tab&embed_desktop=inline&show_copy_link=true" title="Add a Data Source to a Project" frameborder="0" loading="lazy" webkitallowfullscreen mozallowfullscreen allowfullscreen allow="clipboard-write" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; color-scheme: light;" ></iframe></div><!--ARCADE EMBED END-->

---

### 단계

1. **Open Your Project**  
   왼쪽 네비게이션에서 **Projects**를 클릭하고 대상 프로젝트를 선택합니다.

2. **Add a Datasoource**  
   **Datasources**로 이동한 다음 **Add Datasource**를 클릭합니다.

3. **Choose Datasource Type**  
   데이터소스 유형을 선택하세요: Table 또는 View.

4. **Find your Datasource on the list**  
   목록에서 원하는 Datasource를 찾습니다.

5. **Define Snapshot Query**  
   Snapshot Query를 정의합니다. Snapshot query는 *digna*가 하루치 데이터를 접근하는 방식을 정의합니다.

6. **Preview**  
   미리보기를 클릭하여 Snapshot query가 올바르게 정의되었는지 확인합니다.

7. **Create datasource**  
   모든 설정이 올바르게 구성되었다면 구성을 저장합니다.