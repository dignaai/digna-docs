---
title: Panoramica delle connessioni ai database – Configurazione ODBC senza DSN | digna Documentazione
description: Come funzionano le connessioni ai database in digna. Ogni tecnologia sorgente viene raggiunta tramite ODBC con una stringa di connessione senza DSN costruita a partire da proprietà ODBC. Copre l'installazione del driver sull'host digna, la schermata Add DB Connection, la cifratura delle proprietà, il test delle connessioni, la risoluzione dei problemi e i collegamenti alle guide per tecnologia.
...
lang: it
robots: index, follow
og_title: Connessioni ai database digna – Configurazione ODBC senza DSN
og_description: Configura una connessione sorgente digna tramite ODBC senza DSN. Installazione del driver, proprietà ODBC, cifratura, test e risoluzione dei problemi.
