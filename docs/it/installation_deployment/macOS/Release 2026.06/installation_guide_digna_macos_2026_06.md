---
title: Guida all'installazione su macOS – digna Release 2026.06 | Documentazione digna
description: Guida passo passo per l'installazione di digna Release 2026.06 su macOS — requisiti di sistema, configurazione di Homebrew e PostgreSQL, configurazione di nginx o Apache, configurazione del backend e della dashboard, esecuzione di digna come servizio in background e aggiornamento a una nuova release.
keywords: digna installazione macos, guida distribuzione digna mac, configurazione backend digna, installazione dashboard digna, postgresql homebrew, nginx macos, servizio launchd digna, guida aggiornamento digna
...
Questo crea un utente con l'indirizzo e-mail `admin@example.com` e privilegi amministrativi completi.
...
### Passo 6: Avviare il server digna
...
!!! note "Il server occupa il terminale"
    ...
    `serve` viene eseguito in primo piano e continua finché non lo fermate con ++ctrl+c++. Lasciatelo in esecuzione mentre completate la configurazione; per avviarlo automaticamente all'avvio vedi [Eseguire digna come servizio in background](#running-digna-as-a-background-service).
