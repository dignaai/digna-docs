---
title: Обзор единого входа (SSO) | digna Documentation
description: Как работает единый вход (SSO) в digna с использованием OpenID Connect (OIDC). Охватывает настройку в панели и на бэкенде, тестирование, устранение неполадок и ссылки на руководства по провайдерам Microsoft Entra ID, Google Workspace, Okta, Auth0, Keycloak, OneLogin, PingOne и AD FS.
...
Для каждого провайдера должна быть выделенная секция `[oidc_clients.<key>]`. Ключ должен совпадать с `key`, указанным в `dashboard_config.toml`.
