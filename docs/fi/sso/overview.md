---
title: Yhden kirjautumisen (SSO) yleiskatsaus | digna-dokumentaatio
description: Kuinka yhden kirjautumisen (SSO) integraatio toimii dignassa käyttäen OpenID Connectia (OIDC). Kattaa kojelaudan ja backendin määritykset, testauksen, vianetsinnän sekä linkit palveluntarjoajakohtaisiin ohjeisiin Microsoft Entra ID:lle, Google Workspacelle, Oktalle, Auth0:lle, Keycloakille, OneLoginille, PingOnelle ja AD FS:lle.
...
Jokaisella tarjoajalla on oltava oma `[oidc_clients.<key>]`-osionsa. Key-arvon täytyy vastata `dashboard_config.toml`-tiedostossa määriteltyä `key`-arvoa.
